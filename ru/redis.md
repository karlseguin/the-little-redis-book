...
Мы россмотрим более конкретные примеры в дальнейшем. Сейчас, очень важно чтобы мы разобрались в этих базовых принципах Redis. Этот подход позволяет использовать в качестве значений что угодно - Redis никогда не обращается к ним. Мы научимся научимся строить модели наших данных по другому, как того требует этот новый мир.
